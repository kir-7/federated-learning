...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # parameters is stakced and contains all cluster models            
        cluster_models = []
        for i in range(k_clusters):
            start = i * layers_per_model
            end = (i + 1) * layers_per_model
            cluster_models.append(parameters[start:end])

        # get cluster assignment
        assigned_cluster = -1
        min_loss = float('inf')
        
        for i, cluster_weights in enumerate(cluster_models):
            check_loss = self.calculate_loss(cluster_weights, train_loader)
            if check_loss < min_loss:
                min_loss = check_loss
                assigned_cluster = i

        best_parameters = cluster_models[assigned_cluster]
        ...
        return self.get_parameters(config={}), len(train_data), {"train_loss": final_loss, "data_samples":len(train_data), "assigned_cluster":assigned_cluster}
    ...
    def evaluate(self, parameters, config):
        ...
        for i, cluster_weights in enumerate(cluster_models):
            check_loss = self.calculate_loss(cluster_weights, val_loader)
            if check_loss < min_loss:
                min_loss = check_loss
                assigned_cluster = i
